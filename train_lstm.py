...
from models import CraneTiltLSTM
...
print("=" * 70)
print("CRANE STRUCTURAL TILT FORECASTING - LSTM")
print("=" * 70)
print(f"\nDevice: {device}")
...
class CraneWindowDataset(Dataset):
...
train_dataset = CraneWindowDataset(train_store, INPUT_STEPS, FORECAST_STEPS, TRAIN_STRIDE)
val_dataset   = CraneWindowDataset(val_store,   INPUT_STEPS, FORECAST_STEPS, EVAL_STRIDE)
test_dataset  = CraneWindowDataset(test_store,  INPUT_STEPS, FORECAST_STEPS, EVAL_STRIDE)
...
def huber_mse_loss(pred, target, alpha=0.1):
    """MSE + alpha * MAE (robust to outlier tipping steps)."""
